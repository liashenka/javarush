...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        try(ZipOutputStream zipWriter = new ZipOutputStream(new FileOutputStream(zipFileName))){
            for (Map.Entry<String, ByteArrayOutputStream> paar : archivedFiles.entrySet()){
                if(paar.getKey().substring(paar.getKey().lastIndexOf("/") + 1).equals(file.getName())) continue;
                zipWriter.putNextEntry(new ZipEntry(paar.getKey()));
                zipWriter.write(paar.getValue().toByteArray());
            }
